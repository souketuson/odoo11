    ...
    RenderDateRangePicker: function(this2, node) {
        ...
        self.$tm723_time_range.find('.tm723_time_field').daterangepicker({
            ...
            locale : {
                ...
                customRangeLabel: _t('自訂範圍'),
            },
            // .set({hour:0,minute:0,second:0,millisecond:0})
            ranges: {
                '今天': [moment().startOf('day'), moment().endOf('day')],
                '昨天': [moment().startOf('day').subtract(1, 'days'), moment().endOf('day').subtract(1, 'days')],
                '過去7天': [moment().startOf('day').subtract(6, 'days'), moment().endOf('day')],
                '過去30天': [moment().startOf('day').subtract(29, 'days'), moment().endOf('day')],
                '本月份': [moment().startOf('month'), moment().endOf('month')],
                '上月份': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')]
            }
        });
